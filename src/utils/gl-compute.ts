 ...
 * @returns 更新变形后分段三角形顶点数据的回调
 */
 export function genVerticesUpdater1(
   ...
   */
  const updateVertices = (x: number, y: number, vertexYIndex: number) => {
    vertices[vertexYIndex - 1] = x;
    vertices[vertexYIndex] = y;
  };
  /**
   * 接收顶点数据并根据顶点的位置相应规则更新类型数组的回调
   * @param x 顶点x坐标值
  ...
  return (x: number, y: number, xIndex: number, yIndex: number) => {
    if (xIndex > 0 && xIndex < xCount && yIndex > 0 && yIndex < yCount) {
      const c1 = 24 * yIndex * xCount + 24 * xIndex,
        c0 = c1 - 24 * xCount;
      updateVertices(x, y, c0 - 3);
      updateVertices(x, y, c0 + 9);
      updateVertices(x, y, c0 + 17);
      updateVertices(x, y, c1 - 19);
      updateVertices(x, y, c1 - 11);
      updateVertices(x, y, c1 + 1);
    } else if (xIndex === 0 && yIndex === 0) {
      updateVertices(x, y, 1);
    } else if (xIndex === xCount && yIndex === yCount) {
      updateVertices(x, y, xCount * yCount * 24 - 3);
    } else if (xIndex === xCount && yIndex === 0) {
      const c0 = 24 * xIndex;
      updateVertices(x, y, c0 - 19);
      updateVertices(x, y, c0 - 11);
    } else if (xIndex === 0 && yIndex === yCount) {
      const c0 = 24 * xCount * yIndex - 24 * xCount;
      updateVertices(x, y, c0 + 9);
      updateVertices(x, y, c0 + 17);
    } else if (yIndex === 0 && xIndex > 0 && xIndex < xCount) {
      const c0 = 24 * xIndex;
      ...
      const c0 = 24 * xCount * yIndex - 24 * xCount + 24 * xIndex;
      updateVertices(x, y, c0 - 3);
      updateVertices(x, y, c0 + 9);
      updateVertices(x, y, c0 + 17);
    } else if (xIndex === 0 && yIndex > 0 && yIndex < yCount) {
      const c1 = 24 * xCount * yIndex,
        c0 = c1 - 24 * xCount;
      updateVertices(x, y, c0 + 9);
      updateVertices(x, y, c0 + 21);
      updateVertices(x, y, c1 + 1);
    } else {
      const c0 = 24 * xCount * yIndex,
        c1 = c0 + 24 * xCount;
      updateVertices(x, y, c0 - 3);
      updateVertices(x, y, c1 - 19);
      updateVertices(x, y, c1 - 11);
...
export function updatePicVertices1(
  pa: Point2D,
  pb: Point2D,
  pd: Point2D,
  vertices: Float32Array,
